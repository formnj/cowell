<template>
    <modal name="modalSkinImage" height="auto" :class="modalClass">
        <div class="modal_header">
            <h2 v-if="modalTitle">{{ modalTitle }}</h2>
            <button type="button" class="btn_close" @click="$modal.hide('modalSkinImage'), modalClose($event)">닫기</button>
        </div>
        <!-- modal fixe bottom -->
        <slot name="bottom" />
        <!-- //modal fixe bottom -->
